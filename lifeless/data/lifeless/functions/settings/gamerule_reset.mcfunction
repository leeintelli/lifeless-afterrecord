execute in overworld run gamerule announceAdvancements false
execute in overworld run gamerule commandBlockOutput false
execute in overworld run gamerule doImmediateRespawn true
execute in overworld run gamerule doLimitedCrafting true
execute in overworld run gamerule showDeathMessages false
execute in overworld run gamerule spawnRadius 1000

execute in the_nether run gamerule announceAdvancements false
execute in the_nether run gamerule commandBlockOutput false
execute in the_nether run gamerule doImmediateRespawn true
execute in the_nether run gamerule doLimitedCrafting true
execute in the_nether run gamerule showDeathMessages false

execute in the_end run gamerule announceAdvancements false
execute in the_end run gamerule commandBlockOutput false
execute in the_end run gamerule doImmediateRespawn true
execute in the_end run gamerule doLimitedCrafting true
execute in the_end run gamerule showDeathMessages false